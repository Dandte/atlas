...
// GPT models with format conversion
// ═══════════════════════════════════════
...
export class OpenAIProvider implements ModelProvider {
  ...
  constructor(model?: string) {
    ...
    this.model = model ?? config.openaiModel ?? 'gpt-4o';
  }
  ...
  private convertMessages(
    ...
    for (const msg of messages) {
      ...
      if (msg.role === 'assistant') {
        ...
        // User message with potential tool_result and image blocks
        ...
        // Image + text content as array, plain text as string
  ...
  async chatStream(
    ...
    try {
      ...
      logger.error('OpenAI streaming API error', { error: err });
      throw err;
    }
  }
